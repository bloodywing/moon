...
def run():
    parser = argparse.ArgumentParser(description="Pyxel Packager - Moon Version")
    parser.add_argument(
    ...
    vendor_lib_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'vendor', 'dist', 'lib')
    vlibs = filter(os.path.isfile, glob.glob(os.path.join(vendor_lib_dir, "*")))
    for vlib in vlibs:
        libname = os.path.basename(vlib)

        options.append(
            "--add-binary={}{}{}".format(
                os.path.join(vendor_lib_dir, libname), separator, '.'
            )
        )

    assets = filter(os.path.isfile, glob.glob("moongame/assets/**", recursive=True))
